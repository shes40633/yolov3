...
class YoloDatasets(keras.utils.Sequence):
    ...
    def get_random_data(self, annotation_line, input_shape, max_boxes=100, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        #   读取图像并转换成RGB图像
        #------------------------------#
        # image open type PIL.PngImagePlugin.PngImageFile
        # image   = Image.open(line[0])
        # iamge = f"C:/Users/HSIPL/Desktop/yolo3-tf2/VOCdevkit/VOC2007/JPEGImages/1_New-1"
        # data = envi.open(iamge+'.hdr')
        # print(line[0])
        image = envi.open(line[0])
        ...
        # random train 默認true val 默認false
        if not random:
            scale = min(w/iw, h/ih)
            nw = int(iw*scale)
            nh = int(ih*scale)
            dx = (w-nw)//2
            dy = (h-nh)//2
            # print('++ nwh',scale,w,h,iw,ih,nw,nh,dx,dy)
            #---------------------------------#
            #   将图像多余的部分加上灰条
            #---------------------------------#
            # print(image.shape)
            ...
            # print('tet',image.shape)
            new_image = np.zeros((w, h, ib)) + 128/255
            # print('xy',dx,dy)
            if(dx < 0 and dy <0):
                dx = abs(dx)
                dy = abs(dy)
                ...
                dx = abs(dx)
                new_image[dy:dy+nh,...] = image[:,dx:dx+w,:]
            elif(dx >= 0 and dy < 0):
                dy = abs(dy)
                new_image[:,dx:dx+nw,:] = image[dy+h,...]
                
            else:
                # print(dy,nh,dx,nw)
                new_image[dx:dx+iw,dy:dy+ih,:] = image
                # image = new_image[...,2:5]
                # plt.figure()
                # plt.imshow(image)
                # plt.show()
        ...
        if new_ar < 1:
            nh = int(scale*h)
            nw = int(nh*new_ar)
            # print('11',scale,new_ar)
        else:
            nw = int(scale*w)
            nh = int(nw/new_ar)
            # print('22',scale,new_ar)
        # print(nw,nh)
        ...
        # offset = [dx, dy]
        new_image = np.zeros((w, h, ib)) + 128/255
        # print('testimage',new_image.shape)
        # print(new_image.shape, image.shape)
        # print(offset[0],offset[0]+image.shape[0], offset[1],offset[1]+image.shape[1])
        # new_image[offset[0]:offset[0]+image.shape[0], offset[1]:offset[1]+image.shape[1]] = image
        # print('dd',dx,dy,dx+nw,dy+nh)
        if(dx < 0 and dy <0):
            # print('ee')
            dx = abs(dx)
            dy = abs(dy)
            new_image = image[dy:dy+h,dx:dx+w,:]
            # print('11',new_image.shape)
        elif(dx < 0 and dy >= 0 and dy+nh <= 416):
            # print('tt')
            dx = abs(dx)
            new_image[dy:dy+nh,...] = image[:,dx:dx+w,:]
            # print('22',new_image.shape)
        elif(dx >= 0 and dy < 0 and dx+nw <= 416):
            # print('uu')
            dy = abs(dy)
            new_image[:,dx:dx+nw,:] = image[dy:dy+h,...]
            # print('33',new_image.shape)
        elif(dx+nw <= 416 and dy+nh <= 416):
            # print('qq')
            new_image[dy:dy+nh,dx:dx+nw,:] = image
            # print('44',new_image.shape)
        
        # print('newimage',np.where(new_image != 128))
